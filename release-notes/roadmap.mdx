...
### Automation: Second Batch of Improvements

A follow-up release to the April 17 automation overhaul is already in flight and expected in the next release.

- **Dynamic Recipient Resolution** - Target the user assigned to the triggering record (training assignee, ticket owner, task owner, etc.) instead of picking specific users by hand at rule-creation time. Adds an "Assigned User" recipient type to the email and in-app alert pickers that resolves to the right person at send-time.
- **Additional Ticket Template Variables** - Ticket number, assignee name, and a broader set of ticket fields will be available in the Template Variables helper so you can reference them in subject lines and bodies.
