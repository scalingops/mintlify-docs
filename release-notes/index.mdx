...
### Halo PSA Project Sync Overhaul

Project sync for Halo PSA has been rebuilt for reliability and performance. Projects, milestones, child tickets, and ticket actions now sync correctly, with significantly fewer API calls.

- **Split Sync Architecture**: Project sync is now two separate tasks (projects + milestones, then child tickets + actions), making syncs faster and easier to debug
- **Milestone Extraction**: Milestones are now extracted from the embedded project detail response, fixing the previous approach that silently returned no results
- **Action Syncing**: Ticket actions (notes, time entries, status changes) are now fetched alongside each child ticket, providing richer data for AI weekly update summaries
- **Client-Level Filtering**: Projects are filtered by mapped client IDs at the API level, reducing unnecessary API calls by up to 20x for tenants with large Halo instances
- **Fewer API Calls**: Child tickets are now fetched more efficiently, reducing redundant API requests per project
...
- **Automated Weekly Reports**: A scheduled background task generates trend analysis weekly and surfaces insights in AI weekly update notes
...
- **Missing Sync Types**: Contracts, locations, notes, billing items, and project time entries now sync correctly for new Autotask integrations. Previously these five sync types were never triggered during setup
- **Ticket Status Fix**: The `is_closed_status` field on Autotask tickets now sets correctly, fixing a bug where all tickets appeared as open
- **Webhook Overhaul**: Autotask webhooks have been rebuilt with automatic re-provisioning if a webhook breaks
...
- **Background Sync**: New Trigger.dev scheduled task syncs Acronis incidents on a recurring basis with incremental cursor-based fetching
...
- **Dedicated Backup Sync**: Daily sync from NinjaOne's backup usage endpoint captures per-device storage metrics across cloud image, file/folder, and network share backup types
- **Backup Detail Sheet**: Click any NinjaOne device to open a detail panel with cloud, local, and revision storage breakdowns
- **Server-Side Filtering**: The Backup Devices table now filters across the full dataset instead of only the current page, with status and search queries handled server-side
- **Improved Status Display**: Devices without backup configured show "Not Configured" in a neutral badge instead of appearing as failures. The default view shows only devices with active backup.
...
All nine integration wizard flows (Acronis, Cork, Cove, Hudu, IT Glue, Liongard, Pax8, RMM, and ScalePad) now support batch company creation. Previously, creating companies for unmapped integration items happened one at a time, which was slow for large imports. The new bulk action inserts all selected companies in a single database call and auto-maps them to the corresponding integration items.

- **Single Batch Insert**: Creating 50+ companies during an integration setup now completes in seconds instead of minutes
- **Import All with Bulk Create**: The "Import All" action also uses batch creation, combining company creation and mapping saves into optimized bulk operations
- **Automatic Tool Stack Sync**: Newly created companies automatically receive tool stack items via background task
...
- **ITGlue Document Sync**: Improved pagination handling and vendor API library for more reliable document imports across large ITGlue instances
- **Autotask Opportunities Sync**: Added metadata tracking for sync cursor progress
- **RLS Security Audit**: Applied company-scoping fixes to multiple database policies ensuring data isolation across tenants
- **Documentation Loading Skeleton**: Updated to match the new sidebar + table layout, eliminating layout shift during page load
...
Users with location-based ticket permissions now see accurate counts across all dashboard widgets and drilldowns. Previously, dashboard aggregation only handled "all" and "own" visibility modes, returning zero results for users scoped to specific locations. The v3 drilldown worker has also been updated to respect location visibility.
...
Background sync tasks now run with smart concurrency limits per integration provider, preventing performance degradation during large sync operations. Each provider (Autotask, ConnectWise, and others) has tuned concurrency settings that balance sync speed with database stability. Cursor progress writes have been optimized, and transient errors now retry with exponential backoff.

### Realtime Notifications & Approvals

Notifications and approval counts now update instantly using Supabase Realtime. Previously, badge counts relied on polling, which meant delays before new notifications appeared. Now, notification badges and pending approval counts reflect changes the moment they happen, with no page refresh needed.
...
- **Compliance Performance**: Eliminated sequential scans on compliance assignment queries for faster page loads
...
- **Impersonation**: Better client state cleanup when switching between impersonated users
...
- Drilldown queries optimized for sub-100ms response times across all PSA providers
...
- **Autotask**: Location now resolves from `companyLocationID` through the integration locations table
- **Syncro**: Locations now link through integration locations for consistent site naming
- **ConnectWise**: Fixed location resolution to use the correct site ID field
...
- **User Import Linking**: Fixed an issue where ConnectWise user sync created duplicate imported profiles instead of linking to existing auth accounts, which broke team role assignments
- **Imported User Status**: Users incorrectly marked as "imported" who already have active auth accounts are now automatically corrected to "active"
...
- Client-side caching avoids redundant searches
...
- **AI SDK Compatibility**: Updated all AI calls to the latest Vercel AI SDK conventions for continued compatibility
...
- **Report Data Performance**: All report data fetched in a single optimized database call for significantly faster generation
