...
Combined with the existing contact-type to role mapping (ConnectWise, Autotask types 1-5, and Halo user types), this makes role assignment fully deterministic: a "Primary Contact" pseudo-type catches any PSA's primary flag, contact-type rules catch the typed cases, and per-company overrides catch the exceptions. The autotask, ConnectWise, and Halo user-sync background tasks use the same resolver, with shared unit tests.
...
- **Stable when UDFs change**: removed UDFs stay in the definitions table as inactive so existing rules that reference them can flag "this UDF no longer exists" instead of silently breaking
...
- **Company scope**: Blocks no longer carry stale saved company IDs; the company scope now flows purely from the report's context, so reports always reflect the company selector.
- **Error visibility**: Server action and API route failures in the reporting stack are now reported to Sentry with proper tagging.
...
Building a new role from scratch can be tedious, especially when an existing role has most of what you need. The Roles tab in **Settings > Users & Roles** now has a **Duplicate** action on every row that instantly creates a copy named "Copy of [original role]". The copy carries over the description, every permission, and configuration flags like bypass-company-restrictions and update-email opt-in. For company roles, it also copies the assigned ticket display group.
