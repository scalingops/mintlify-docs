...
The "Sync from planner" button on **Budgets, Forecast** now actually populates the forecast table, and recurring planner items finally project the right way through the years ahead. Three issues that were quietly making the sync produce nothing useful are all fixed in one go.

- **Sync from planner now fills in values**: A category-key mismatch was causing the forecast page to ignore everything the planner returned, even when there was clearly planner data for the company. Click "Sync from planner" again and the rows actually populate now.
- **Monthly costs annualize correctly**: A planner item priced at $200 per month now shows as $2,400 in each forecast year instead of $200. Annual recurring items contribute their value once per year. One-time costs still appear only in the year they are scheduled.
- **Recurring items project across the whole forecast horizon**: A subscription that started in 2025 still appears in 2026, 2027, and 2028 if those years are in your forecast window. Previously it only showed up in the year of the planner item's due date.
- **Mixed cost lines work end-to-end**: A planner item with both a one-time setup fee and a monthly recurring cost (for example, $5,000 setup + $200/month) now contributes both lines correctly. The setup lands in the year of the due date, and the monthly line annualizes and repeats through the forecast.

### Add to Budget Preserves Recurrence

The "Add to budget" button on the planner sidebar inside the budget editor now keeps the planner item's recurrence (Monthly, Annual, or One-Time) instead of always defaulting to One-Time. This means a $200/month subscription on the planner page becomes a Monthly line item with annualized cost $2,400 in the budget, exactly as you would expect. Items with multiple cost lines (one-time plus recurring) create one budget line per cost, each labeled with its recurrence.

[Documentation: Budgets](/user-guides/budgets#budget-forecast)

### AI Narrative Reports Use Your Company's Currency

AI-generated narrative reports now render every monetary value in the report subject's currency instead of always defaulting to USD. A report on a Canadian client shows CAD, a UK client shows GBP, and a German client shows EUR, resolved automatically from the company's country (with your tenant's country as a fallback). The figures you read in the narrative match the figures your client sees in their own accounting system, with no surprise conversions. Applies to manual generation, scheduled reports, and retries.

### New Budgets Default to the Company's Currency

Creating a new budget for a Canadian client now opens with the currency pre-set to CAD; a UK client opens to GBP; a German client to EUR. The currency picker resolves from the selected company's country (with the tenant's country as a fallback) instead of unconditionally starting at USD, so you stop catching the wrong currency after the first row of data has already been typed in.
