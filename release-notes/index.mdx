...
### Automation Rules: PSA-Aware Tickets, Custom Fields, and Expanded Targeting

The automation rule builder got a major overhaul aimed at making rules faster to configure and far harder to misconfigure. Text fields that used to demand you paste in raw IDs are now guided dropdowns populated from your actual data, custom fields are now first-class citizens in both conditions and actions, and tickets are now an entity you can build rules around directly.

**PSA-aware ticket creation:**

The **Open Ticket** action now adapts to your connected PSA. Instead of a generic text box for board, category, impact, urgency, priority, or status, you get dropdowns populated from what is actually configured in your PSA instance.

- **Halo PSA**: dropdowns for Category, Impact, and Urgency pulled from your Halo lookup lists
- **Autotask**: dropdowns for Issue Type and Sub-Issue Type, with Sub-Issue Type filtered to valid children of the selected Issue Type
- **ConnectWise**: dropdowns for Service Board and ticket Type
- PSA terminology (Board vs Queue vs Team, Agreement vs Contract, Member vs Resource vs Agent) automatically matches your integration

No more guessing at integer IDs or copying values out of your PSA admin screens. If a value is selectable in your PSA, it is selectable in the rule builder.

**Custom fields in conditions and actions:**

Every custom field you have defined for an entity is now available when building an automation rule:

- **In conditions**: Filter rules using any custom field. Text fields support contains/equals/starts-with, number and currency fields support greater/less than, date and datetime fields support date thresholds, and select, radio, priority, impact, and urgency fields populate a dropdown of their configured options
- **In update actions**: The Update Field action now allows writing to `custom_fields` across devices, companies, compliance items, planner items, users, goals, tickets, and meetings
- **As date triggers**: Custom date fields can drive date-threshold automations (e.g., "30 days before a Contract Renewal Date custom field")

Multiselect and radio fields pull their options from your custom field definition, so dropdowns always reflect exactly what your team has configured.

**Tickets are now a first-class entity type:**

Automation rules can now target tickets directly. Build rules that watch ticket status, priority, age, or custom fields, then run update actions, send emails, create planner items, or hand off to the AI transform pipeline. Available across all triggers (date threshold, scheduled, field change, absence).

**Expanded recipient options for Send Email and In-App Alert:**

Previously recipients were limited to specific internal users. You can now target any of:

- **Specific Users**: Pick internal MSP team members individually
- **Company Teams**: Team members assigned to the affected company in selected roles (Account Manager, vCIO, etc., using the new PSA contact-type mapping)
- **Company Contacts**: Contacts synced from the affected company's PSA record
- **Meeting Attendees**: All participants of the triggering meeting
- **Custom Email**: Enter any external email address

Each option has a guided picker so you are never typing in role IDs or UUIDs by hand.

**Dynamic dropdowns everywhere:**

Status, device type, priority, impact, urgency, manufacturer, and other lookup fields now fetch their options from the database instead of being hardcoded. Values always match what is configured in your tenant, and when you add a new status or priority in settings, it shows up in the rule builder automatically.

**Test-before-enable Review step:**

The final step of the rule builder now runs a dry-run test against your live data and shows exactly which entities would match, with a breakdown of the fields the rule inspected. Catch mistakes before the rule ever fires.

**Approval workflow:**

Actions can now require approval before execution. When turned on, the rule generates a pending approval (with the rendered email body, planner item, or field changes it would apply), notifies the selected users or role, and waits. Approvers review the exact generated content, then approve or reject. Useful for AI-generated content, high-impact field updates, and anything you want a human check on before it goes live.

<Info>
**More automation updates shipping next week.** A second batch of automation improvements is already in flight for the next release, including:

- **Dynamic recipient resolution** — target the user assigned to the triggering record (training assignee, ticket owner, task owner, etc.) without picking specific users by hand at rule-creation time
- **Additional ticket template variables** including ticket number and assignee
- **Weekly day-of-week selector** for Scheduled rules so you can pin a weekly run to a specific weekday

We ran out of runway to land these in the April 17 release, but they are active development work and should land in the next release.
</Info>
